...
@WebServlet("/hod_Update")
@MultipartConfig(maxFileSize = 160000000) 
public class hod_Update extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (row > 0) {
                response.sendRedirect("hod_EditProfile.jsp?message=Profile updated successfully");
            } else {
                response.getWriter().println("<script>alert('Records not updated!'); location.href='hod_EditProfile.jsp';</script>");
            }
