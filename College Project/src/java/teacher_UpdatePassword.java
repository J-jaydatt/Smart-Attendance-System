...
@WebServlet("/teacher_PasswordUpdate")
public class teacher_UpdatePassword extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        catch (NumberFormatException e) 
        {
            ...
            response.sendRedirect("teacher_EditProfile.jsp?error=Invalid ID");
            return;
        }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (!real_Password.equals(old_Pass)) {
                    ...
                    response.sendRedirect("teacher_EditProfile.jsp?fail=Failed to update Password");
                    return;
                }
                ...
                if (!new_Pass.equals(confirm_Pass)) {
                    ...
                    response.sendRedirect("teacher_EditProfile.jsp?match=Password Doesn't Match");
                    return;
                }
                ...
                if (rowsAffected > 0) {
                    response.sendRedirect("teacher_EditProfile.jsp?success=Password Changed Successfully");
                } else {
                    response.sendRedirect("teacher_EditProfile.jsp?fail=Failed to update Password");
                }
            } else {
                // If no teacher found with the given ID
                response.sendRedirect("teacher_EditProfile.jsp?fail=Teacher not found");
            }
        } catch (SQLException e) {
            e.printStackTrace();
            response.sendRedirect("teacher_EditProfile.jsp?error=Database Error");
        } finally {
            try {
                if (connection != null) {
                    connection.close();
