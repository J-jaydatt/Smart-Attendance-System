...
@WebServlet("/student_PasswordUpadate")
public class student_PasswordUpadate extends HttpServlet {
    @Override
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (!real_Password.equals(old_Pass)) {
                    ...
                    response.sendRedirect("student_EditProfile.jsp?fail=Failed to update Password");
                    return;
                }
                ...
                if (!new_Pass.equals(confirm_Pass)) {
                    ...
                    response.sendRedirect("student_EditProfile.jsp?match=Password Doesn't Match");
                    return;
                }

                // Update the password if all checks pass
                String updateQuery = "UPDATE student SET password = ? WHERE sId = ?";
                ...
                if (rowsAffected > 0) {
                    response.sendRedirect("student_EditProfile.jsp?success=Password Changed Successfully");
                } else {
                    response.sendRedirect("student_EditProfile.jsp?fail=Failed to update Password");
                }
            } else {
                // If no teacher found with the given ID
                response.sendRedirect("stundent_EditProfile.jsp?fail=Teacher not found");
            }
