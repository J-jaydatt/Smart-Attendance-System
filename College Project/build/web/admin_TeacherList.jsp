...
form span {
    ...
    <script>
          ...
          function indexpage()
    {
        window.location.href="admin_TeacherOperations.jsp";
    }
    ...
    <h2 id="tdyses">Teacher Records</h2>
    ...
    <%
        String sql4 = "SELECT tId, tName, tGender, tContact, tEmail, verified FROM teacher";
    ...
    <tr>
        ...
        <%  int status=rs.getInt("verified");
        
            if(status==1)
            {
        %>
        <td style="color:green; font-style: italic">Verified</td>
        <% }
        else{
        %>
        <td><button type="submit" style="margin-left:15px; ">Verify</button></td>
        <%}%>
        <form action="admin_TeacherProfileEdit.jsp">
            <input type="hidden" value="<%= rs.getInt("tId") %>" name="Sid">
